...
form.action = savedEngine;

// 更新线条状态
function updateLineState(input) {
	const xian = input.nextElementSibling.nextElementSibling;
	input.setCustomValidity('');
	xian.classList.toggle('active', input.value.trim() !== '');
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const xian = document.querySelector('.xian');
	const searchEngineMenu = document.querySelector('.search-engine-menu');
	const searchEngineItems = document.querySelectorAll('.search-engine-menu ul li');

	xian.classList.toggle('active', input.value.trim() !== '');

	// 设置默认选中的搜索引擎
	searchEngineItems.forEach(item => {
		if (item.getAttribute('data-engine') === savedEngine) {
			item.classList.add('active');
		}
	});
	...
	searchEngineItems.forEach(item => {
		item.addEventListener('click', () => {
			...
			form.action = engine; // 更新表单 action

			// 根据搜索引擎切换参数名称
			if (engine.includes('baidu.com')) {
				input.name = 'wd';  // 百度使用wd参数
			} else {
				input.name = 'q';   // 其他引擎使用q参数
			}

			localStorage.setItem('searchEngine', engine); // 存储到 localStorage
			searchEngineItems.forEach(i => i.classList.remove('active'));
			item.classList.add('active');
			searchEngineMenu.classList.remove('open');
			document.querySelector('.engine-toggle').classList.remove('active');
		});
	});

	// 初始化时检查当前引擎
	if (savedEngine.includes('baidu.com')) {
		input.name = 'wd';
	} else {
		input.name = 'q';
	}
	...
	// 点击页面其他区域隐藏菜单
	document.addEventListener('click', (e) => {
		if (!e.target.closest('.search-engine-menu') && !e.target.closest('.engine-toggle')) {
			searchEngineMenu.classList.remove('open');
			document.querySelector('.engine-toggle').classList.remove('active');
		}
	});
...
// 菜单跳转与缓动
function smoothScrollTo(targetY, duration = 800) { // 🔹 调整 duration 控制滚动时间，单位 ms
    ...
    function animationStep(currentTime) {
        ...
        let progress = Math.min(elapsedTime / duration, 1); // 🔹 控制滚动进度，确保不超过 1

        // 使用 easeInOutCubic 缓动函数让滚动更流畅
        let easedProgress = easeInOutCubic(progress); // 🔹 这里控制滚动的平滑度
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        ...
        let targetName = this.getAttribute('href').substring(1);
        let target = document.getElementsByName(targetName)[0];

        if (target) {
            let offset = 50; // 🔹 这里调整滚动时顶部的偏移量
            let targetPosition = target.getBoundingClientRect().top + window.scrollY - offset;
            smoothScrollTo(targetPosition, 500); // 🔹 这里的 600 代表滚动时间，调小更快，调大更慢
        }
    });
});
 ...
 // 计算网站运行的天数
 const launchDate = new Date("2022-11-17");
 const timeDifference = currentDate - launchDate;
 const totalDaysSinceLaunch = Math.floor(timeDifference / (1000 * 60 * 60 * 24));

 // 格式化月份和日期，确保是两位数（01, 02, ..., 09）
